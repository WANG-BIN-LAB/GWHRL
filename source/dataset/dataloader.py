...
def init_dataloader(cfg: DictConfig,
    ...
    train_dataloader = utils.DataLoader(
        train_dataset, batch_size=cfg.dataset.batch_size,  drop_last=cfg.dataset.drop_last)

    val_dataloader = utils.DataLoader(
        val_dataset, batch_size=cfg.dataset.batch_size,  drop_last=False)

    test_dataloader = utils.DataLoader(
        test_dataset, batch_size=cfg.dataset.batch_size,  drop_last=False)
...
def init_stratified_dataloader(cfg: DictConfig,
    ...
    val_dataset = utils.TensorDataset(
        final_timeseires_val,
        final_pearson_val,
        labels_val,
    )

    test_dataset = utils.TensorDataset(
        final_timeseires_test,
        final_pearson_test,
        labels_test,
    )
    ...
    test_dataloader = utils.DataLoader(
        test_dataset, batch_size=cfg.dataset.batch_size,  drop_last=False)
    # print(f"Train length: {len(train_dataloader.dataset)}")
    # print(f"Validation length: {len(val_dataloader.dataset)}")
    # print(f"Test length: {len(test_dataloader.dataset)}")
    return [train_dataloader, val_dataloader, test_dataloader]
