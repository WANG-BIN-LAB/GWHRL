...
class DEC(nn.Module):
    ...
    def forward(self, batch: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # [batch size, node_num, cluster_number]， 正交聚类，对应论文中软聚类 P
        ...
        # [batch size, cluster_number, hidden dimension] ，P转置与transformer编码器输出Zl乘法，对应论文中图水平嵌入Zg，
        # 把 node_num 个节点根据聚类权重压缩成了 cluster_num 个簇,用于后续做分类
        node_repr = torch.bmm(assignment.transpose(1, 2), encoded)

        return node_repr, assignment  # 返回图节点表示Zg(batch_size, cluster_num, node_num)和正交聚类权重P(batch_size, node_num, cluster_num)
